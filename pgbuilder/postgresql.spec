...
Source0:        %{source0}
Source1:        %{sname}-%{pgmajorversion}-pg_config.h
Source2:        %{sname}-%{pgmajorversion}-ecpg_config.h
Source3:        %{sname}-%{pgmajorversion}-setup
Source4:        %{sname}-%{pgmajorversion}-check-db-dir
Source5:        %{sname}-%{pgmajorversion}.service
%if 0%{?suse_version}
Source6:        %{sname}-%{pgmajorversion}.pam.suse
%else
Source6:        %{sname}-%{pgmajorversion}.pam
%endif
Source7:        %{sname}-%{pgmajorversion}-tmpfiles.d
Source8:        %{sname}-%{pgmajorversion}-libs.conf
Source9:        %{sname}-%{pgmajorversion}-Makefile.regress
Source10:       %{sname}-%{pgmajorversion}-README-systemd.rpm-dist
Source11:       https://www.postgresql.org/files/documentation/pdf/%{pgmajorversion}/%{sname}-%{pgmajorversion}-A4.pdf
...
Provides:	dby%{pgmajorversion}-server
...
set +e
export SHELLOPTS
./configure "$@"' > ./configure.rpmbuild

bash ./configure.rpmbuild --enable-rpath \
...
sed -e 's|^PGVERSION=.*$|PGVERSION=%{pgmajorversion}|' \
	...
	<%{SOURCE3} >postgresql-%{pgmajorversion}-setup
...
sed -e 's|^PGVERSION=.*$|PGVERSION=%{pgmajorversion}|' \
	...
	<%{SOURCE4} >%{sname}-%{pgmajorversion}-check-db-dir
touch -r %{SOURCE4} %{sname}-%{pgmajorversion}-check-db-dir
%{__install} -m 755 %{sname}-%{pgmajorversion}-check-db-dir %{buildroot}%{pgbaseinstdir}/bin/%{sname}-%{pgmajorversion}-check-db-dir

%{__install} -d %{buildroot}%{_unitdir}
%{__install} -m 644 %{SOURCE5} %{buildroot}%{_unitdir}/%{sname}-%{pgmajorversion}.service
%endif

%if %pam
%{__install} -d %{buildroot}/etc/pam.d
%{__install} -m 644 %{SOURCE6} %{buildroot}/etc/pam.d/%{sname}
%endif
...
%{__install} -m 0644 %{SOURCE7} %{buildroot}/%{_tmpfilesdir}/%{sname}-%{pgmajorversion}.conf
%endif
...
%{__install} -m 700 %{SOURCE8} %{buildroot}%{pgbaseinstdir}/share/
...
%if %test
	...
	popd
	%{__cp} %{SOURCE9} %{buildroot}%{pgbaseinstdir}/lib/test/regress/Makefile
...
%if 0%{?pgmajorversion} >= 17
cat pg_combinebackup-%{pgmajorversion}.lang pg_walsummary-%{pgmajorversion}.lang >> pg_devel.lst
%endif
...
%changelog
* Sat Oct 28 2023 Sebastiaan Mannem <sebas@mannemsolutions.nl> alpha1 spec
- Initial spec for building DBY-hacker PostgreSQL
